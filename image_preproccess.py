...
def SIFT_process(image: np.ndarray):
    picked_num = 20
    # Step 2: Initialize the SIFT detector
    sift = cv2.SIFT_create(contrastThreshold=0.02, edgeThreshold=30)

    # Step 3: Detect keypoints and compute descriptors
    keypoints, descriptors = sift.detectAndCompute(image, None)

    # Step 4: Sort keypoints by response (strongest first)
    keypoints = sorted(keypoints, key=lambda x: x.response, reverse=True)

    # Step 5: Select the top 20 keypoints (if there are more than 20)
    if len(keypoints) > picked_num:
        keypoints = keypoints[:picked_num]
        descriptors = descriptors[:picked_num]
    elif len(keypoints) < picked_num:
        # If fewer keypoints, duplicate the last point to compensate
        image_with_keypoints = cv2.drawKeypoints(
        image, keypoints, None, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS )
        print(len(keypoints))
        cv2.imshow("hello",image)
        cv2.waitKey(0)
        while len(keypoints) < picked_num:
            keypoints.append(keypoints[-1])  # Duplicate the last keypoint
            descriptors = np.vstack([descriptors, descriptors[-1]])  # Duplicate the last descriptor

    # Step 6: Visualize the top 20 keypoints
    image_with_keypoints = cv2.drawKeypoints(
        image, keypoints, None, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS
    )

    # Step 7: Display the result
    cv2.imshow("Top 20 SIFT Keypoints", image_with_keypoints)
    # cv2.waitKey(0)
...
def detect_signs(image:np.ndarray):
    # Load the image
    ...
    circles = cv2.HoughCircles(
        blurred, cv2.HOUGH_GRADIENT, dp=1.2, minDist=50, param1=50, param2=30, minRadius=50, maxRadius=100
    )
    ...
    if circles is not None:
        ...
        x = circles[index_max,0]
        y = circles[index_max,1]
        r = circles[index_max,2]
