...
class MinimalVideoSubscriber(Node):
	...
	def __init__(self):		
		...
		self._video_publisher=self.create_publisher(
				msg_type=Image,
				topic='/image_raw/bgr_format',
				qos_profile=image_qos_profile
		)
		self._video_publisher
	...
	def _image_callback(self, CompressedImage):	
		...
		if(self._display_image):
			...
			# Convert the BGR image (NumPy array) to a ROS Image message    
			ros_image=CvBridge().cv2_to_imgmsg(self._imgBGR,"bgr8")
			self._video_publisher.publish(ros_image)
